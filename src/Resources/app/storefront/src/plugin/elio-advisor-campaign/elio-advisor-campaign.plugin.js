...
export default class ElioAdvisorCampaignPlugin extends window.PluginBaseClass {
    ...
    init () {
        ...
        this.listing.$emitter.subscribe('Listing/afterRenderResponse', (event) => {
            if (this._values.answerPath === null) {
                // if not answerPath -> remove all listing elelements (but not the listing itself because of js plugin on it)
                const listingElem = this.listing.el.querySelector(this.options.listingSelector);
                while (listingElem.firstChild) {
                    listingElem.removeChild(listingElem.firstChild);
                }
            }
    ...
    getValues() {
        ...
        if (this._values.answerPath) {
            values['elio-data-discovery-answer-path-' + this._values.campaignId] = this._values.answerPath
        }
        return values
    }
